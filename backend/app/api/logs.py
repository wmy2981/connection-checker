"""日志查看与导出：按时间范围与级别范围过滤每日日志文件。

日志行格式（见 app/logging_setup.py）：
    2026-08-10 10:30:36,123 | INFO | app.scheduler | 消息文本
    ...
    for f in sorted(_log_dir(request).glob("app-*.log")):
        ...
        with fh:
            for raw in fh:
                ...
                entry = {"time": ts, "level": level, "name": name, "message": message}
                last = entry
                yield entry


def _filtered(
    request: Request, level: str | None, start: str | None, end: str | None
):
...
async def list_logs(
    ...
    items = list(_filtered(request, level, start, end))
    items.reverse()  # 最新在前
    total = len(items)
...
async def export_logs(
    ...
    lines = []
    for entry in _filtered(request, level, start, end):
        lines.append(
